...
DECLARE @records TABLE(
    ...
    DateFin date,
    MandatId int,
    DateMandatInstant int, 
    DateMandatYear int,
    DateMandatMonth int,
    DateMandatWeek int,
    UrlMandat varchar(max),
    StatutMandat varchar(max),
    DateCompromis date,
    BudgetMaxEuro int,
    TypeBien varchar(max),
    TypeProjet varchar(max),
    SurfaceMin int,
    NombrePieces varchar(max),
    NombreChambres varchar(max),
    Ville varchar(max),
    CodePostal int,
    Departement varchar(max),
    Region varchar(max),
    NombreDeBiensTrouves int, 
    NombreDeBiensPublies int,
    NombreDeBiensSelectionnes int,
    NombreDeBiensAVisiter int, 
    NombreDeBiensVisites int,
    NombreDeBiensRejetes int,
    NombreDeBiensActesOuCompromis int, 
    DatePremierePublicationDeBien date,
    DateDernierePublicationDeBien date,
    DelaiPremierePublicationDeBien int,
    DatePremiereVisite date,
    DelaiPremiereVisite int
)
DECLARE @dateFrom datetime
DECLARE @dateTo datetime

SET @dateFrom = '2021-06-01'
SET @dateTo = '2021-06-03'

WHILE(@dateFrom < @dateTo)
BEGIN
    SELECT @dateFrom = DATEADD(day, 1,@dateFrom)
    INSERT INTO @records
    -- Rajouter les valeurs pour chaque colonne
    SELECT @dateFrom, dataset.DateMandat, dataset.DateFinMandat, dataset.MandatId, dataset.DateMandatInstant, dataset.DateMandatYear , 
    dataset.DateMandatMonth, dataset.DateMandatWeek, dataset.UrlMandat, dataset.StatutMandat, dataset.DateCompromis, dataset.BudgetMaxEuro, dataset.TypeBien, dataset.TypeProjet,
     dataset.SurfaceMin, dataset.NombrePieces, dataset.NombreChambres, dataset.Ville, dataset.CodePostal, dataset.Departement, dataset.Region, dataset.NombreDeBiensTrouves, dataset.NombreDeBiensPublies,
     dataset.NombreDeBiensSelectionnes, dataset.NombreDeBiensAVisiter, dataset.NombreDeBiensVisites, dataset.NombreDeBiensRejetes, dataset.NombreDeBiensActesOuCompromis,
     dataset.DatePremierePublicationDeBien, dataset.DateDernierePublicationDeBien, dataset.DelaiPremierePublicationDeBien, dataset.DatePremiereVisite, dataset.DelaiPremiereVisite
    FROM (
        SELECT TOP (100000)
        DATEDIFF(day, CONVERT(DATE, '2012-04-25'), base_dataset.DateMandat) as 'DateMandatInstant'
        ,CASE
            WHEN MONTH(base_dataset.DateMandat) < 3 OR (MONTH(base_dataset.DateMandat) = 3 AND DAY(base_dataset.DateMandat) < 21) THEN 1
            WHEN (MONTH(base_dataset.DateMandat) = 3 AND MONTH(base_dataset.DateMandat) >= 21) OR MONTH(base_dataset.DateMandat) < 6 OR (MONTH(base_dataset.DateMandat) = 6 AND DAY(base_dataset.DateMandat) < 21) THEN 2
            WHEN (MONTH(base_dataset.DateMandat) = 6 AND MONTH(base_dataset.DateMandat) >= 21) OR MONTH(base_dataset.DateMandat) < 9 OR (MONTH(base_dataset.DateMandat) = 9 AND DAY(base_dataset.DateMandat) < 21) THEN 3
            WHEN (MONTH(base_dataset.DateMandat) = 9 AND MONTH(base_dataset.DateMandat) >= 21) OR MONTH(base_dataset.DateMandat) < 12 OR (MONTH(base_dataset.DateMandat) = 12 AND DAY(base_dataset.DateMandat) < 21) THEN 4
            ELSE 1
        END as 'DateMandatSeason'
        ,DATEDIFF(year, CONVERT(DATE, '2012-04-26'), base_dataset.DateMandat) as 'DateMandatYear'
        ,MONTH(base_dataset.DateMandat) as 'DateMandatMonth'
        ,DATEPART(week, base_dataset.DateMandat) as 'DateMandatWeek'
        ,*
        FROM (
            SELECT TOP(100000)
            mandats.[MandatId],
            ...
            FROM (
                ...
                END as 'DateMandat',
                CASE 
                    WHEN m.[DateCompromisSignature] IS NOT NULL THEN CONVERT(DATE, m.[DateMandatSignature])
                    WHEN m.[DateMandatPerte] IS NOT NULL THEN CONVERT(DATE, m.[DateMandatPerte])
                    ELSE CONVERT(DATE, m.[LastSavedDate])
                END as 'DateFinMandat',
                ...
                GROUP BY m.[MandatId], m.[CritereId], m.[StatutProspectMandatClientId], m.[DateMandatSignature], m.[DateCompromisSignature], m.[DateCreation], m.[LastSavedDate], m.[DateMandatPerte]
            ...
            WHERE (@dateFrom >= mandats.[DateMandat] AND @dateFrom <= mandats.[DateFinMandat]) AND mandats.[DateFinMandat] >= mandats.[DateMandat] AND c.[BudgetMaxEuro] > 10 AND ((c.[TypeBienEnum] <> 8 AND c.[SurfaceMin] < 10000) OR (c.[TypeBienEnum] = 8)) AND c.[BudgetMaxEuro] <> 0 AND c.[TypeBienEnum] <> 0 AND LEN(c.[TypeProjet]) <> 0 AND c.[NombrePiecesEnum] <> 0 AND c.[NombreChambresEnum] <> 0
            ) as base_dataset
